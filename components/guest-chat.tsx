  ...
  return (
    <main className="site-shell mx-auto flex min-h-screen max-w-lg flex-col px-4 py-6 sm:px-5">
      ...
      <form onSubmit={submitTranslation} className="order-2 paper-panel mt-6 p-4 sm:p-5">
        ...
        <label className="mt-5 grid gap-1.5 text-sm font-extrabold text-[#162338]">Message or reply<textarea required value={text} onChange={(event) => setText(event.target.value)} rows={4} placeholder="Paste a guest message, choose a phrase, or write your reply" className="resize-y rounded-xl border-[#c9d9df] bg-[#fbfdfd] px-3 py-3 text-sm leading-6 shadow-none" /></label>
      ...
      <section className="order-5 mt-6 pb-6"><div className="flex items-baseline justify-between"><h2 className="text-xl font-extrabold">Saved replies</h2>{messages.length ? <span className="text-xs font-bold text-[#5e7085]">{messages.length}</span> : null}</div>{messages.length ? <ul className="mt-3 grid gap-3">{messages.map((message) => <li key={message.id} className="register-panel p-4"><div className="min-w-0"><p className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#5e7085]">{languages[message.originalLang as LanguageCode]} → {languages[message.translatedLang as LanguageCode]}</p><div className="mt-2 rounded-xl rounded-tl-sm bg-[#f0f5f7] px-3 py-2 text-sm leading-6 text-[#35465b]">{message.originalText}</div><div className="mt-2 rounded-xl rounded-tr-sm bg-[#e5f3ee] px-3 py-2 text-base font-extrabold leading-6 text-[#17634d]">{message.translatedText}</div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => copyReply(message)} className="min-h-9 rounded-md !border-[#9dbec9] !bg-white px-3 text-xs font-bold !text-[#15506d] !shadow-none hover:!bg-[#eef8fb]">Copy reply</button><button type="button" onClick={() => shareReply(message)} className="min-h-9 rounded-md !bg-[#15506d] px-3 text-xs font-bold !text-white">Share reply</button><button type="button" onClick={() => removeMessage(message)} className="min-h-9 rounded-md !border-red-200 !bg-red-50 px-3 text-xs font-bold !text-red-700 !shadow-none hover:!bg-red-100">Remove</button></div></div></li>)}</ul> : <div className="mt-3 rounded-xl border border-dashed border-[#c8dce0] bg-white/50 px-4 py-5 text-center"><p className="text-sm font-bold text-[#35465b]">No replies yet.</p><p className="mt-1 text-xs leading-5 text-[#5e7085]">Translated replies will stay here for easy reuse and sharing.</p></div>}</section>
    </main>
  );
}
