...
export function GuestChat() {
  ...
  return (
    <main className="site-shell mx-auto flex min-h-screen max-w-lg flex-col px-4 py-6 sm:px-5">
      ...
      <section className="order-3 mt-4 rounded-2xl border border-[#cce3db] bg-[#f4faf7] p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-extrabold">Quick replies</h2><p className="mt-1 text-xs text-[#4e7568]">Offline phrases and reusable details</p></div><button type="button" onClick={() => setShowQuickReplies((current) => !current)} className="min-h-9 rounded-lg !border-[#b8d5c8] !bg-white px-3 text-xs font-extrabold !text-[#17634d] !shadow-none">{showQuickReplies ? "Hide" : "Choose phrase"}</button></div>{showQuickReplies && <><div className="mt-3 flex flex-wrap gap-2">{offlinePhrasePacks.map((pack) => <button key={pack.id} type="button" onClick={() => setActivePack(pack.id)} className={`min-h-8 rounded-full px-3 py-1 text-xs font-extrabold !shadow-none ${activePack === pack.id ? "!border-[#17634d] !bg-[#17634d] !text-white" : "!border-[#b8d5c8] !bg-white !text-[#17634d] hover:!bg-[#fafffc]"}`}>{pack.label}</button>)}</div><div className="mt-3 flex flex-wrap gap-2">{offlinePhrasePacks.find((pack) => pack.id === activePack)?.phrases.map((phrase) => <button key={phrase} type="button" onClick={() => { useQuickPhrase(phrase); setShowQuickReplies(false); }} className="min-h-9 rounded-full !border-[#b8d5c8] !bg-white px-3 py-1.5 text-left text-xs font-bold !text-[#17634d] !shadow-none hover:!border-[#76a993] hover:!bg-[#fafffc]">{phrase}</button>)}</div><div className="mt-4 grid gap-2 border-t border-[#d4e5dc] pt-3"><div className="grid grid-cols-[1fr_auto] gap-2"><input value={quickPrice} onChange={(event) => setQuickPrice(event.target.value)} inputMode="numeric" placeholder="Nightly price, e.g. 2500" className="min-h-9 rounded-lg border-[#b8d5c8] bg-white px-2 text-xs" /><button type="button" disabled={!quickPrice.trim()} onClick={() => useTemplate(`The price is ₹${quickPrice.trim()} per night.`)} className="min-h-9 rounded-lg !border-[#b8d5c8] !bg-white px-2 text-xs font-extrabold !text-[#17634d] !shadow-none">Use price</button></div><div className="grid grid-cols-[1fr_auto] gap-2"><input value={quickDate} onChange={(event) => setQuickDate(event.target.value)} placeholder="Arrival date, e.g. 12 October" className="min-h-9 rounded-lg border-[#b8d5c8] bg-white px-2 text-xs" /><button type="button" disabled={!quickDate.trim()} onClick={() => useTemplate(`Please arrive on ${quickDate.trim()}.`)} className="min-h-9 rounded-lg !border-[#b8d5c8] !bg-white px-2 text-xs font-extrabold !text-[#17634d] !shadow-none">Use date</button></div></div></>}</section>
