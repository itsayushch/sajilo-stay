  ...
  return (
    <main className="site-shell mx-auto min-h-screen max-w-lg px-4 py-6">
      ...
      <form onSubmit={submitTranslation} className="paper-panel mt-6 p-4">
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2"><label className="grid gap-1 text-sm font-bold">From<select value={sourceLanguage} onChange={(event) => setSourceLanguage(event.target.value as LanguageCode)} className="min-h-10 rounded-lg border border-slate-300 px-2">{Object.entries(languages).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label><button type="button" onClick={swapLanguages} className="mb-1 min-h-10 rounded-lg px-2 font-bold text-[#1f5d3b] underline" aria-label="Swap languages">⇄</button><label className="grid gap-1 text-sm font-bold">To<select value={targetLanguage} onChange={(event) => setTargetLanguage(event.target.value as LanguageCode)} className="min-h-10 rounded-lg border border-slate-300 px-2">{Object.entries(languages).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label></div>
        ...
        <button type="submit" disabled={isTranslating} className="mt-4 min-h-10 rounded-lg bg-[#1f5d3b] px-3 font-bold text-white disabled:opacity-60">{isTranslating ? "Translating…" : "Translate"}</button>
      </form>
