...
import { downloadListingModel, isListingModelDownloaded } from "@/lib/listing";
...
export function OfflineAiDownload({ onContinue }: { preferredLanguage: LanguageCode; onContinue?: () => void }) {
  ...
  useEffect(() => {
    isListingModelDownloaded().then((downloaded) => {
      setIsReady(downloaded);
      if (downloaded) {
        setProgress(100);
