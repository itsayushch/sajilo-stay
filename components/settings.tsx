...
export function Settings() {
  ...
  async function downloadTranslation() {
    ...
    try {
      await downloadTranslationModels(sourceLanguage, targetLanguage, ({ progress, file }) => {
        setDownloadProgress(progress);
        if (file) setDownloadStatus(`Downloading ${file.split("/").pop()} · ${progress}%`);
      });
  ...
  async function downloadListingWriter() {
    ...
    try {
      await downloadListingModel(({ progress, file }) => {
        setDownloadProgress(progress);
        if (file) setDownloadStatus(`Downloading ${file.split("/").pop()} · ${progress}%`);
      });
