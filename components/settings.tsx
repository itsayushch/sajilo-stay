...
export function Settings() {
  ...
  return <main className="site-shell mx-auto min-h-screen max-w-lg px-4 py-6"><ConnectionStatus /><Link href="/" className="home-link inline-flex min-h-11 items-center text-sm font-bold">← Back</Link><header className="mt-6"><p className="text-sm font-bold text-[#1f5d3b]">Make the app yours</p><h1 className="mt-1 text-3xl font-bold">Settings</h1><p className="muted-copy mt-2 text-base leading-6">Control which AI tier Sajilo Stay may use.</p></header><section className="paper-panel mt-6 p-4"><label className="flex cursor-pointer items-start gap-4"><input type="checkbox" checked={enabled} onChange={toggle} className="mt-1 h-6 w-6 accent-[#1f5d3b]"/><span><span className="sign-title block text-xl font-bold">Force offline mode</span><span className="muted-copy mt-1 block text-sm leading-5">Skip Groq even when this phone has a connection. On-device AI and offline basics still work.</span></span></label></section><p className="status-line mt-5 text-sm font-bold" role="status">{status}</p>{profile ? <section className="mt-8"><h2 className="text-2xl font-bold">Your homestay profile</h2><ProfileForm profile={profile} onSaved={setProfile} submitLabel="Save profile changes" /></section> : profile === null ? <p className="muted-copy mt-8 text-sm">Complete onboarding from Home to add your homestay profile.</p> : null}</main>;
}
