...
| --- | --- | --- | --- | --- |
| Guest translation | Groq `openai/gpt-oss-20b` through a server-side Next.js route | Chrome Translator API, when supported | — | A Nepali/Hindi/Bengali phrasebook for common availability, price, directions, food, and house-rule messages |
| Listing writing | Groq `openai/gpt-oss-20b` | Chrome Prompt API / Gemini Nano, when supported | Cached browser-run SmolLM2 135M instruction model (optional ~180 MB Lite AI download) | A factual listing template based on the host notes, plus a deterministic price band |

Sajilo Stay automatically uses online AI when it is reachable. If online or device AI is unavailable, the relevant static fallback is used automatically.
...
5. Translate `is the room available?` and generate a listing from short notes. The saved phrase and listing template must remain usable without a network request.
